...
function openQR(path){
  ...
  const url = new URL(location.href); url.pathname = url.pathname.replace(/\/$/, '/') + path;
  if (!navigator.onLine) {
  ...
  dlg.showModal();
  $('#qrCopyBtn').onclick = async ()=>{ try{ await navigator.clipboard.writeText(url.toString()); }catch{ alert(url.toString()); } };
...
function render(){
  ...
  $('#list').onclick = async (e)=>{
    ...
    if(share){
      ...
      url.pathname = url.pathname.replace(/\/$/, '/') + share.dataset.share;
      try{
